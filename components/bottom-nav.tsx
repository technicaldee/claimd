...
const items = [
  ...
  { href: "/post", label: "Post", icon: "add_box", key: "post" },
...
export function BottomNav({ active }: { active: string }) {
  return (
    <nav className="fixed bottom-0 left-0 z-50 flex w-full items-center justify-around border-t border-outline-variant/15 bg-white/90 px-4 pb-6 pt-3 backdrop-blur md:hidden">
      {items.map((item) => {
